    ...
    return suite = {
      ...
      '[NoCR] Testing RootNode set property': {
        topic: function() {
          ...
          return getSessionAsync(function(err, session) {
            return session.getRootNode(function(err, node) {
              return self.callback(err, node);
            });
          });
        },
        "Trying to set a string property without specifying type": function(err, node) {
          node.setProperty("myproperty", "mavaleur");
          return assert.ok(node.getProperty("myproperty") instanceof nocr.Property, "Error at node.getProperty() instanceof Property");
        },
        "Testing value of previously setted property": function(err, node) {
          return assert.ok(node.getProperty("myproperty").getString() === "mavaleur", "Error, incorrect string value returned");
        },
        "Testing if getProperty throws error (with no callback)": function(err, node) {
        ...
        "Testing if property callback send an error": function(err, node) {
          var assertfunc, checkerr;
          assertfunc = function() {
